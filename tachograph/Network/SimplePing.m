...
- (void)startWithHostAddress
{
    ...
    else
    {
        ...
        /* Return Value: The new CFSocket object, or NULL if an error occurred. If a CFSocket object already exists for sock, the function returns the pre-existing object instead of creating a new object; the context, callout, and callBackTypes parameters are ignored in this case. */
        ...
        CFRunLoopAddSource(CFRunLoopGetCurrent(), rls, kCFRunLoopDefaultMode);//将 CFRunLoopSourceRef 添加到 RunLoop 的 kCFRunLoopDefaultMode 模式中。
...
- (void)hostResolutionDone {
    ...
    //调用 CFHostGetAddressing 函数来获取解析结果。这个函数返回 CFDataRef 对象的一个数组，其中的每一个都包含一个 POSIX 的 sockaddr 结构体。
...
static void HostResolveCallback(CFHostRef theHost, CFHostInfoType typeInfo, const CFStreamError *error, void *info) {
    ...
    // 在系统解析 HostName 成功之后会调用 HostResolveCallback 这个回调
    // 这个回调的作用相当于重定向，将内容从 c 转成适当的 Objective-C 内容。
    ...
    assert(typeInfo == kCFHostAddresses);//第二个参数设置为 kCFHostAddresses 表明想要返回一个 IP 地址
    
    if ( (error != NULL) && (error->domain != 0) ) {
        [obj didFailWithHostStreamError:*error];
    } else {
        [obj hostResolutionDone];// 在这个方法获取 HostName 对应的地址
    }
}
